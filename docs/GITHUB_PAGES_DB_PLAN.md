...
원본 로컬 프로젝트는 React 화면, Python API, KBO 명단 수집기와 SQLite로 동작한다. 이 배포 복사본은 React가 Supabase에 직접 연결되도록 바꾸었다. `automation/sync_supabase.py`와 GitHub Actions가 KBO 명단 및 전날 조건별 번호를 채운다.
...
5. `Sync KBO data`를 수동 실행한 뒤 Pages 배포본에서 팀 변경·추첨·재추첨을 확인한다.
